...
function App() {
      ...
      const resizeObserver = new ResizeObserver(() => {
        if (containerRef.current) {
          ...
          window.electronAPI.updateContentDimensions?.({ 
            width: Math.min(scrollWidth + 32, 600), 
            height: Math.min(scrollHeight + 32, 800) 
          });
        }
      });
  ...
  return (
    <div 
      ref={containerRef}
      className="w-full h-full bg-black bg-opacity-90 backdrop-blur-lg border border-gray-600 rounded-2xl overflow-hidden shadow-2xl flex flex-col"
      style={{ minWidth: '400px', minHeight: '500px', maxWidth: '600px', maxHeight: '800px' }}
    >
      {/* Header */}
      ...
      {!isMinimized && (
        <>
          {/* Tab Navigation */}
          <div className="flex border-b border-gray-700 bg-gray-800 bg-opacity-30">
            <button
                ...
                <div className="flex-1 p-4 overflow-y-auto">
                  {messages.length === 0 ? (
                    <div className="text-center text-gray-400 mt-8">
                      ...
                      <p className="text-sm">Welcome to Local AI Agent Chat</p>
